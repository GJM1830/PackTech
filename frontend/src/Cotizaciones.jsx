...
function FormularioCotizacion({ onCreada, duplicarDesde, editando, onCancelarEdicion }) {
  ...
  // aparte, identificada por el ID de esa cotización, así nunca se mezcla con el borrador de
  // "Nueva Cotización" (eso era lo que causaba el error de "código duplicado"). Al reabrir esa
  // misma cotización para editarla, si hay un borrador pendiente se recupera automáticamente.
  useEffect(() => {
    if (editandoId) {
      guardarFiltros(CLAVE_BORRADOR_EDICION_COTIZACION, { id: editandoId, form, items, itemActual, procesosItemActual })
    } else {
  ...
  useEffect(() => {
    if (editando) {
      ...
      // Si hay un borrador guardado de una sesión anterior de edición sobre ESTA MISMA
      // cotización (por ID), se recupera tal cual quedó, en vez de partir de los datos
      // ya guardados en el servidor — así no se pierde nada si el operador se salió
      // a mitad de editar.
      let borradorEdicion = null
      try {
        const guardado = localStorage.getItem(CLAVE_BORRADOR_EDICION_COTIZACION)
        if (guardado) {
  ...
  const manejarEnvio = async (e) => {
    ...
    try {
      ...
      try { localStorage.removeItem(CLAVE_BORRADOR_EDICION_COTIZACION) } catch { /* no crítico */ }
